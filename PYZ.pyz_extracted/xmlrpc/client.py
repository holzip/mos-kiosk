...
try:
        ...
        def __dump(self, value, write):
            ...
            try:
                ...
                for type_ in type(value).__mro__:
                    if not type_ in self.dispatch.keys():
                        continue
                    raise TypeError('cannot marshal %s objects' % type(value))
    ...
    '''
        ...
        def __init__(self, use_datetime, use_builtin_types = (False, False)):
            self._type = None
            self._stack = []
            self._marks = []
            self._data = []
    ...
    class _MultiCallMethod:
        ...
        def __getattr__(self, name):
            return _MultiCallMethod(self._MultiCallMethod__call_list, f'''{self._MultiCallMethod__name!s}.{name!s}''')
    ...
    raised in response to xmlrpc faults.'''
        ...
        def __getitem__(self, i):
            ...
            if isinstance(item, list):
                return item[0]
            raise None('unexpected type in multicall result')
    ...
    def getparser(use_datetime, use_builtin_types = (False, False)):
    ...
    '''
        ...
        target = None(use_datetime = use_datetime, use_builtin_types = use_builtin_types)
        if FastParser:
            parser = FastParser(target)
            return (parser, target)
        parser = None(target)
    ...
    def dumps(params, methodname, methodresponse, encoding, allow_none = (None, None, None, False)):
    ...
    def loads(data, use_datetime, use_builtin_types = (False, False)):
    ...
    '''
        if not gzip:
            raise NotImplementedError
        gzf = gzip.GzipFile(mode = 'rb', fileobj = BytesIO(data))
        if max_decode < 0:
            decoded = gzf.read()
        else:
    ...
    GzipDecodedResponse = <NODE:27>(GzipDecodedResponse, 'GzipDecodedResponse', gzip.GzipFile if gzip else object)
    ...
    class _Method:
        ...
        def __getattr__(self, name):
            return _Method(self._Method__send, f'''{self._Method__name!s}.{name!s}''')
    ...
    class Transport:
        ...
        def __init__(self = None, use_datetime = (False, False), use_builtin_types = {
            'headers': () }, *, headers):
            self._use_datetime = use_datetime
        ...
        def request(self, host, handler, request_body, verbose = (False,)):
            for i in (0, 1):
                ...
                return (0, 1), self.single_request(host, handler, request_body, verbose)
            return None
        ...
        def make_connection(self, host):
            ...
            (chost, self._extra_headers, x509) = None.get_host_info(host)
    ...
    '''
        ...
        def __init__(self, uri, transport, encoding, verbose, allow_none = None, use_datetime = (None, None, False, False, False, False), use_builtin_types = {
            'headers': (),
            'context': None }, *, headers, context):
            p = urllib.parse.urlsplit(uri)
            if p.scheme not in ('http', 'https'):
                raise OSError('unsupported XML-RPC protocol')
            self._ServerProxy__host = p.netloc
            self._ServerProxy__handler = None(urllib.parse.urlunsplit)
        ...
        def __repr__(self):
            return f'''<{self.__class__.__name__!s} for {self._ServerProxy__host!s}{self._ServerProxy__handler!s}>'''
        ...
        '''
            ...
            raise None(f'''Attribute {attr!r} not found''')
