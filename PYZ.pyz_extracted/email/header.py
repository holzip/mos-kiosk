    ...
    def __str__(self):
        ...
        for string, charset in self._chunks:
            ...
            if uchunks:
                ...
                if not (lastcs not in (None, 'us-ascii') or nextcs in (None, 'us-ascii')) and hasspace:
                    uchunks.append(SPACE)
                    nextcs = None
                elif not nextcs not in (None, 'us-ascii') and lastspace:
                    uchunks.append(SPACE)
            if string:
                string
            ...
            for ch in self._splitchars:
                for i in range(self._current_line.part_count() - 1, 0, -1):
                    if ch.isspace():
                        ...
                        if fws and fws[0] == ch:
                            range(self._current_line.part_count() - 1, 0, -1)
                        else:
                            prevpart = self._current_line[i - 1][1]
                            if not prevpart:
                                continue
                            if not prevpart[-1] == ch:
                                continue
                            self._splitchars
