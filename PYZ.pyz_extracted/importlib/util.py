...
class _incompatible_extension_module_restrictions:
    ...
    *will* happen.
    ...
    override = (lambda self: if self.disable_check:
-1)()
    ...
    def __getattribute__(self, attr):
        ...
        if object.__getattribute__(self, '__class__') is _LazyModule:
            if loader_state['is_loading']:
                None(None, None)
                return 
            object.__getattribute__(self, '__dict__') = None
            ...
            for key, value in attrs_now.items():
                if key not in attrs_then:
                    ...
                    continue
                if not id(attrs_now[key]) != id(attrs_then[key]):
                    continue
                attrs_updated[key] = value
...
class LazyLoader(Loader):
    ...
    __check_eager_loader = (lambda loader: if not hasattr(loader, 'exec_module'):
raise TypeError('loader must define exec_module()'))()
    factory = (lambda cls, loader: pass# WARNING: Decompyle incomplete
)()
