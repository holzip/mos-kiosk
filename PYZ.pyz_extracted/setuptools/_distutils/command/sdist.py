...
class sdist(Command):
    ...
    def checking_metadata(self = None):
    ...
    def get_file_list(self = None):
        ...
        if template_exists and self._manifest_is_not_generated():
            self.read_manifest()
            self.filelist.sort()
    ...
    _cs_path_exists = (lambda fspath: if not os.path.exists(fspath):
Falseabspath = os.path.abspath(fspath)(directory, filename) = os.path.split(abspath)filename in os.listdir(directory))()
    ...
    def _add_defaults_standards(self):
        ...
        for fn in standards:
            if isinstance(fn, tuple):
                ...
                got_it = False
                for fn in alts:
                    if not self._cs_path_exists(fn):
                        continue
                    got_it = True
    ...
    def read_template(self = None):
        ...
        template = TextFile(self.template, strip_comments = True, skip_blanks = True, join_lines = True, lstrip_ws = True, rstrip_ws = True, collapse_join = True)
    # WARNING: Decompyle incomplete
    ...
    def prune_file_list(self = None):
        ...
        vcs_dirs = [
            ...
            '\\.bzr',
            '_darcs']
        vcs_ptrn = '(^|{})({})({}).*'.format(seps, '|'.join(vcs_dirs), seps)
    ...
    def write_manifest(self = None):
        ...
        self.execute(file_util.write_file, (self.manifest, content), f'''writing manifest file \'{self.manifest}\'''')

    
    def _manifest_is_not_generated(self):
        if not os.path.isfile(self.manifest):
            return False
        fp = open(self.manifest, encoding = 'utf-8')
        first_line = next(fp)
        None(None, None)
    ...
    def read_manifest(self = None):
        ...
        lines = open(self.manifest, encoding = 'utf-8')
        self.filelist.extend(filter(None, filterfalse(is_comment, map(str.strip, lines))))
        None(None, None)
        return None
        with None:
            if not None:
    ...
    def make_release_tree(self = None, base_dir = None, files = None):
