...
# File: pyside_tool.pyc (Python 3.12)
...
def main():
    ...
    command = [
        os.path.join(os.path.dirname(os.path.realpath(__file__)), cmd)]
...
def designer():
    ...
    elif sys.platform == 'win32' and is_virtual_env():
...
def _check_requirements(requirements_file):
    ...
    missing_packages = []
    file = open(requirements_file, 'r', encoding = 'UTF-8')
    for line in file:
...
def android_deploy():
    if sys.platform == 'win32':
        print('pyside6-android-deploy only works from a Unix host and not a Windows host', file = sys.stderr)
        return None
